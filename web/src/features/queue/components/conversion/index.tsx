...
export default function Conversion(props: ConversionProps) {
  ...
  function remove() {
    app.state.dispatch(app.state.reducers.queue.remove(props.id));
      ...
      <td>
        <Dropdown active={actionsMenuActive} onClose={() => setActionsMenuActive(false)}>
          ...
          <DropdownMenu side="right">
            ...
            <DropdownMenuItem onClick={() => remove()} color="danger" startContent={<FaRegTrashCan />}>Remove</DropdownMenuItem>
          </DropdownMenu>
        </Dropdown>
      </td>
    </>
